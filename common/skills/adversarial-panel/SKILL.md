...
- **Panelists** — 2–4 reviewers. Maximize heterogeneity in this order:
  1. Different Claude models via the Agent tool `model` parameter (e.g. mix
     `opus` + `sonnet` + `haiku`). Default and preferred: this buys real
     weight-level diversity — different training runs and sizes carry
     genuinely different blind spots — while keeping every panelist
     competent enough that its critiques carry verification advantage
     instead of noise.
  2. Same model with forced-divergent *methods* (not just tones): one argues
     from first principles, one from base rates / outside view, one hunts
     only for disconfirming evidence, one re-executes verifiable claims. Use
     when more panelists are wanted than distinct Claude models make sense
     for.
  3. External CLI (e.g. `codex`, `gemini`) as a supplementary panelist only
     — check availability first (e.g. `codex --version`). Prompting the
     same Claude weights differently shifts style and focus, not the
     systematic blind spots baked in by training, so tier 1 is where real
     decorrelation comes from. Non-Claude models available today are
     generally weaker at the kind of judgment calls this panel reviews
     (differentiation, design consistency, scope judgment), so a weak
     external critique can inject noise the facilitator then has to filter
     back out. Reach for this tier when the user explicitly wants a
     genuinely different-family opinion, or when the claim under review is
     itself cheaply verifiable by reproduction (so a weak reviewer's "this
     fails on input X" still carries weight regardless of its judgment
     quality).
...
- **Only one model family available**: force method-level divergence (tier 2
  above) and downgrade the evidential weight of convergence in your
  synthesis.
