...
- **切り口に詰まったら先に `/last30days <アプリのジャンル>` を回す。** 直近 30 日の Reddit / HN /
  YouTube の実発言を engagement 順で返す（API キー不要）。想像で「刺さりそうな悩み」を書く前に、
  実際に言われている言い回しを取る。取れた語彙はそのまま本文・ハッシュタグの種にする。
