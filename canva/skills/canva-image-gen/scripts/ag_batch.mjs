...
const APPS=[['sakezukan','5Z6N5CT278'],['sushizukan','2X647T52W7'],['eigazukan','D795U57L3Z'],['seizazukan','K37M93393X'],['michinoekizukan','K37MFBXS5Y']];
...
async function assign(nm,id){
  await page.goto('https://developer.apple.com/account/resources/identifiers/bundleId/edit/'+id,{waitUntil:'networkidle',timeout:35000}).catch(()=>{});
  await page.waitForTimeout(3500);
  // App Groups checkbox 有効化
  ...
  await page.waitForTimeout(3500);
  console.log(nm.padEnd(16), 'cfg='+cfg, 'pick='+pick, sv, '-> url:', page.url().includes('/list')?'LIST(ok)':page.url().slice(-40));
}
for(const [nm,id] of APPS){ try{ await assign(nm,id); }catch(e){ console.log(nm,'ERR',e.message); } }
await b.close().catch(()=>{});
